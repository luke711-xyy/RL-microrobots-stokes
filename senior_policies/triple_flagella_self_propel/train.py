...
parser = argparse.ArgumentParser(description="Train triple-flagella senior policy")
...
class TrainingMetricsCallback(DefaultCallbacks):
    def on_episode_end(self, *, worker, base_env, policies, episode, env_index, **kwargs):
        ...
        episode.custom_metrics["dx12_penalty"] = float(getattr(env_ref, "last_dx12_penalty", 0.0))
        episode.custom_metrics["dx23_penalty"] = float(getattr(env_ref, "last_dx23_penalty", 0.0))
        episode.custom_metrics["dy12_penalty"] = float(getattr(env_ref, "last_dy12_penalty", 0.0))
        episode.custom_metrics["dy23_penalty"] = float(getattr(env_ref, "last_dy23_penalty", 0.0))
        episode.custom_metrics["dx12"] = float(getattr(env_ref, "last_dx12", 0.0))
        episode.custom_metrics["dx23"] = float(getattr(env_ref, "last_dx23", 0.0))
        episode.custom_metrics["dy12"] = float(getattr(env_ref, "last_dy12", 0.0))
        episode.custom_metrics["dy23"] = float(getattr(env_ref, "last_dy23", 0.0))
...
def write_training_run_markdown(run_dir, cli_args, trainer_config):
    env_params = {
        ...
        "macro_horizon": swimmer_module.MACRO_HORIZON,
        "low_level_hold_steps": swimmer_module.LOW_LEVEL_HOLD_STEPS,
        "formation_target_dx12": swimmer_module.FORMATION_TARGET_DX12,
        "formation_target_dx23": swimmer_module.FORMATION_TARGET_DX23,
        "formation_target_dy12": swimmer_module.FORMATION_TARGET_DY12,
        "formation_target_dy23": swimmer_module.FORMATION_TARGET_DY23,
        "forward_reward_coef": swimmer_module.FORWARD_REWARD_COEF,
        "dx12_penalty_coef": swimmer_module.DELTA_X12_PENALTY_COEF,
        "dx23_penalty_coef": swimmer_module.DELTA_X23_PENALTY_COEF,
        "dy12_penalty_coef": swimmer_module.DELTA_Y12_PENALTY_COEF,
        "dy23_penalty_coef": swimmer_module.DELTA_Y23_PENALTY_COEF,
        "observation_dim": 22,
        "macro_action_num": len(MACRO_ACTION_TABLE),
        "macro_action_table": MACRO_ACTION_TABLE,
        "observation_definition": "[robot1_summary(6), robot2_summary(6), robot3_summary(6), dx12, dy12, dx23, dy23]",
    ...
    lines = [
        "# Triple Senior Training Run Parameters",
        "",
...
def main():
    ...
    for sub_dir in ("traj", "traj2", "trajp"):
        os.makedirs(os.path.join(os.getcwd(), sub_dir), exist_ok=True)
