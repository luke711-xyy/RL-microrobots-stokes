...
MAX_STEP=10000
DT = 0.01
PRESSURE_REWARD_COEF = 8.0
TURN_REWARD_COEF = 500.0
...
#        print(self.x_tar)
        ...
        self.order=0
        self.escape_count=0
        self.turn_direction = env_config.get("turn_direction")
        if self.turn_direction not in ("cw", "ccw"):
            raise ValueError("env_config['turn_direction'] must be 'cw' or 'ccw'")
        self.target_turn_sign = -1.0 if self.turn_direction == "cw" else 1.0
...
#         reward+=(((r)/N)*100)      
        ...
        if len(self.centroid_history) >= 201:
            ...
            if pressure_active and recent_norm > 1e-8 and old_norm > 1e-8:
                self.last_signed_turn = compute_signed_turn_angle(old_vec, recent_vec)
                self.last_signed_turn_deg = math.degrees(self.last_signed_turn)
                self.last_turn_scale = min(recent_norm / self.displacement_gate_ref, 1.0)
                self.last_turn_reward = (
                    TURN_REWARD_COEF
                    * self.target_turn_sign
                    * (self.last_signed_turn / math.pi)
                    * self.last_turn_scale
                )
        ...
        if self.ep_step % 200 == 0:
            print(f"  [Step {self.ep_step:>4d}] Centroid: ({self.state_n[0]:.4f}, {self.state_n[1]:.4f}) | "
                  ...
                  f"P_rwd: {self.last_pressure_reward:>8.4f}, Turn_rwd: {self.last_turn_reward:>9.4f}, "
                  f"TurnDeg100: {self.last_signed_turn_deg:>8.2f}, Disp100: {self.last_recent_displacement:>7.4f}, "
                  f"Gate100: {self.displacement_gate_ref:>7.4f}, TurnScale: {self.last_turn_scale:.3f}, "
                  f"TargetDir: {self.turn_direction}")
...
# #             reward+=self.X*100#done = True
    ...
    def reset(self):
        if self.ep_step > 0:
            ...
            print(f"[Episode {self.episode_count:>4d}/~2000] ep_reward: {self.reward:.4f}")
